var api = frameElement.api, oper = api.data.oper, id = api.data.id, companyList = SYSTEM.companyList,
    $_form = $("#base_form"), addNew = false,haveCompany,tree=[];
var model = avalon.define({
    ...
    setIs_head: function (v) {
        model.data.is_head = v;
    },
    setM: function (v) {
        model.data.m = v;
    },
    ...
    showMenu: function (code) {
        ...
        if (api.data.type == 0 && !haveCompany) {
            /**上级部门 combo START*/
            var r = $("#parent_depart");
            var i = Public.comboTree(r, {
                width: 260,
                offset: {left: 122, top: 0},
                url: rootPath + '/sso/position/departMentTree.json',
                postData: {},
                callback: {
                    beforeClick: function (e, t) {
                        if (t.id != model.data.id) {
                            r.val(t.name);
                            model.data.parent_id = (t.id);
                            i.hide();
                        } else {
                            r.val("");
        ...
        if (api.data.type == 1  && !haveCompany) {
            /**上级岗位combo START*/
            var r = $("#position");
            var i = Public.comboTree(r, {
                width: 260, offset: {left: 122, top: 0}, url: rootPath + '/sso/position/tree.json',
                callback: {
                    beforeClick: function (e, t) {
    ...
    changeCompany:function(){
        ...
        if (api.data.type == 0) {
            ...
            var i = Public.comboTree(r, {
                ...
                offset: {left: 122, top: 0},
                url: rootPath + '/sso/position/departMentTree.json',
                postData: {companyId: model.data.company_id},
                callback: {
                    beforeClick: function (e, t) {
                        if (t.id != model.data.id) {
                            r.val(t.name);
...
function postData() {
    ...
    Public.ajaxPost(rootPath + "/sso/position/save.json", model.data.$model, function (t) {
        if (200 == t.status) {
            ...
            parent.THISPAGE.reloadData({companyId:model.data.company_id});
            if (addNew) {
                model.init();
            } else {
                api.close();
