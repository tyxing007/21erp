var api = frameElement.api, oper = api.data.oper, id = api.data.id, custParame = SYSTEM.custParame, typeList = custParame.typeList,
    $_form = $("#base_form"), addNew = false, type = parent.type, productListBySuggest,productBsSuggest;
...
function postData() {
    ...
    if(model.data.out_depot_id == model.data.to_depot_id){
        parent.parent.Public.tips({
            type: 1,
            content: '调出仓库和调入仓库不能为同一个仓库'
        });
        return;
    }
    var proudctList = model.data.productlist;
    var len = proudctList.length;
    if(len == 0){
        parent.parent.Public.tips({
            type: 1,
            content: '请输入产品,没有产品无法调拨'
        });
        return;
    }

    for(var i=0;i<len;i++){
        if(proudctList[i].allotmoney == undefined || proudctList[i].allotmoney == ""){
            parent.parent.Public.tips({
                type: 1,
                content: '请输入产品调拨价,否则无法调拨'
            });
            return;
        }
    }
