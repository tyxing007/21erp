...
public class Contacts extends Model<Contacts> {
     ...
     * @return
     */
    public Contacts findFirstByCustomerId(String customer_id) {
        return dao.findFirst("select * from " + tableName + " where customer_id=? ", customer_id);
    }
    ...
    public Page<Contacts> pageGrid(int pageNo, int pageSize, Map<String, Object> filter) {
        ...
        if (StringUtils.isNotEmpty(company_id)) {
            sql.append(" and cust.company_id=? ");
            parame.add(company_id);
        }
