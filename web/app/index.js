...
var model = avalon.define({
    ...
    init: function () {
        ...
        $("#logoutBtn").click(function () {
            $.dialog.confirm("确定要退出二一科技中小企业管理软件吗？未保存的数据将会丢失，请先保存再点击退出！", function () {
...
function reloadCache(){
    ...
    /** 系统客户**/
    Public.ajaxPost(rootPath + "/sso/position/getCompanyTree.json", {}, function (json) {
        SYSTEM.companyList = json.data;
    });

    /** 获取类别参数**/
    Public.ajaxPost(rootPath + "/sso/parame/getCustParameType.json", {}, function (json) {
        for(var i=0;i<json.data.toString().split(",").length;i++){
            CONFIG.custParameType.push(json.data.toString().split(",")[i]);
        }
    });
}