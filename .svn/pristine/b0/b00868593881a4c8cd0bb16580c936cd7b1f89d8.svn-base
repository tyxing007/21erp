...
import net.loyin.utlis.FileUtils;
...
//				FileBox.dao.save();
                ...
                //sql.append(" (name,creater_id,file_path,create_datetime,fsize,save_path,relation_id) values ('?','?','?','?',?,'?','?')");


//                context.Request.Form["ctrlid"];
                String cid = this.getPara("cid");
                Date date = new Date();
                SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
                Map<String, Object> map = new HashMap<String, Object>();
                map.put("name", filedataFileName);
                map.put("creater_id", uid);
                map.put("file_path", realPath + savefilename);
                map.put("create_datetime", df.format(date).toString());
                map.put("fsize", fsize);
                map.put("save_path", realPath);
                map.put("relation_id", cid);

                new FileBean().saveFile(map);
    ...
    public void upImg() {
        ...
        String cid = this.getPara("customer_id");
        String savefilename = "";
        StringBuffer files = new StringBuffer();
        List<UploadFile> upfilelist = this.getFiles();
        Map<String, Object> jsonMap = new HashMap<String, Object>();
        ...
        if (upfilelist != null && upfilelist.isEmpty() == false) {
            ...
            for (UploadFile upfile : upfilelist) {
                ...
                savefilename = idGenerater.to62(new Date().getTime()) + "." + filetype;
                ...
                FileUtils.saveFile(realPath + uploadroot + subdir + "/", savefilename, file);
