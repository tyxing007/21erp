...
public class Position extends Model<Position> {
	...
	public Position findByName(String name,String company_id){
		return dao.findFirst("select a.* from "+tableName+" a where a.name=? and a.company_id=? order by a.sort_num asc",name,company_id);
	}
