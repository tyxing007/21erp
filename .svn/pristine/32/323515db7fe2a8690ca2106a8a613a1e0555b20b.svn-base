...
var model = avalon.define({
    $id: 'view',
    query:  {
        keyword: "", start_date: '', end_date: '', type: -1, qryType: -1, status: 0, is_deleted: 0, uid: "",companyId:sysCompanyId
    }, expoutUrl: "",
    fastQryText: "快速查询",
    fastQry: [
        //{text:"全部",sl:false},
...
//	       {text:"",sl:true},
    ...
    showMenu: function (code) {
        ...
        if(!haveCompany&&first){
            ...
            /**负责人combo END*/
            first = false;
        }else if(haveCompany&&first){
            /**负责人*/
            var r = $("#headCombo");
            var i = Public.comboTree(r, {
                width: 260, url: rootPath + '/sso/user/userTree.json?type=2',postData:{companyId:model.query.companyId}, callback: {
                    beforeClick: function (e, t) {
                        if (t.type == 10) {
                            r.val(t.name);
                            model.query.uid = t.id;
    ...
    loadGrid: function () {
        ...
        $("#grid").jqGrid({
            ...
            colModel: [{
                ...
                name: "type",
                label: "类型", formatter: function (v, e, r) {
                    return type[v];
                },
                align: "center",
    ...
    reply: function (e) {
        Public.ajaxPost(url + "/reply.json", {id: e}, function (t) {
            if (t && 200 == t.status) {
                ...
                parent.Public.tips({
                    type: 1, content: "恢复客户失败！" + t.msg
                });
        });
    },
    trash: function (e) {
        Public.ajaxPost(url + "/trash", {id: e,companyId:model.query.companyId}, function (t) {
            if (t && 200 == t.status) {
                parent.Public.tips({type: 2, content: t.msg});
                THISPAGE.reloadData();
            } else
    ...
    allot: function (id) {
        ...
        $.dialog({
            ...
            init: function () {
                var r = $("#allotToUid");
                var i = Public.comboTree(r, {
                    width: 260, offset: {left: 80}, url: rootPath + '/sso/user/userTree.json?type=2', callback: {
                        beforeClick: function (e, t) {
                            if (t.type == 10) {
                                r.val(t.name);
                                uid = t.id;
