...
import com.sun.net.ssl.internal.www.protocol.https.DelegateHttpsURLConnection;
...
public class OrderProduct extends Model<OrderProduct> {
    ...
    public OrderProduct findByOrderIdAndProductId(String orderId, String productId, String companyId) {
        return this.findFirst("select t.*,p.name product_name,p.unit from " + tableName + " t," + Product.tableName + " p where p.id=t.product_id and t.id=? and t.product_id = ? and p.company_id = ? order by p.name ", orderId, productId, companyId);
    }

    public void updateOrderInfo(String orderId, String productId, String num, String price) {
        Db.update("UPDATE " + tableName + " SET sale_price = '" + price + "', amount = '" + num + "' WHERE id = '" + orderId + "' and product_id = '" + productId + "'");
    }

    public void updateOrderInfo(String orderId, String productId, String num, String price,String amt,String remark) {
        Db.update("UPDATE " + tableName + " SET sale_price = " + price + ", amount = " + num + ", amt = " + amt + ", description = '" + remark + "' WHERE id = '" + orderId + "' and product_id = '" + productId + "'");
    }
    ...
    public List<OrderProduct> findBySloID(String slo_id) {
        return find("select * from " + tableName + " where id= '" + slo_id + "'");
    }
     ...
     * @param product_id
     */
    public void deleteByProductId(String id, Object product_id) {
        Db.update("delete from " + tableName + " where id=? and product_id = ?", id, product_id);
    }
}
