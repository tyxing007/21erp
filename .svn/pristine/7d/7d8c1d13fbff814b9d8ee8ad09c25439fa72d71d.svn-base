...
public class StockCtrl extends AdminBaseController<Stock> {
	...
	public void dataGrid() {
		...
		filter.put("brand",this.getPara("brand"));
		...
		List<Stock> stockList = page.getList();
		for(Stock stock : stockList){
			List<Comparison> comparisonList = Comparison.dao.getComparisonByProductId(stock.getStr("product_id"));
			Comparison comparison = new Comparison();
			Parame parame = Parame.dao.findById(stock.getStr("unit"),this.getCompanyId());
			...
			stock.put("comparison",comparisonList);
		}
		...
		try {
			...
			if(sheet.getLastRowNum()>1){
				for(int idx=2;idx<=sheet.getLastRowNum();idx++){
					...
					map.put("billsn",row.getCell(0).getStringCellValue());
					map.put("product_name",row.getCell(1).getStringCellValue());
					map.put("depot_name",row.getCell(3).getStringCellValue());
					map.put("amount",row.getCell(2).getNumericCellValue());
					map.put("cost",row.getCell(4).getNumericCellValue());
					dataList.add(map);
				}
