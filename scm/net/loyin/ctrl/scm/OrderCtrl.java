...
public class OrderCtrl extends AdminBaseController<Order> {
    ...
    public void qryOp() {
        ...
        if (m != null) {
            OrderSend orderSend = OrderSend.dao.getOrderSend(id);
            if (orderSend == null) {
                m.put("people", "");
                m.put("phone", "");
                m.put("address", "");
            } else {
                m.put("people", orderSend.getStr("people"));
                m.put("phone", orderSend.getStr("phone"));
                m.put("address", orderSend.getStr("address"));
            }
            List<OrderProduct> productlist = OrderProduct.dao.list(id);
            for(OrderProduct orderProduct : productlist){
                ProductHis productHis = ProductHis.dao.getProductHisBySomeId(this.getCompanyId(),orderProduct.getStr("product_id"),m.getStr("customer_id"));
                if(productHis == null){
                    orderProduct.put("his_price",0);
                    orderProduct.put("his_purchase_price",0);
                }else{
                    orderProduct.put("his_price",productHis.getStr("his_price"));
                    orderProduct.put("his_purchase_price",productHis.getStr("his_purchase_price"));
                }
            }
    ...
    public void subAudit() {
        ...
        try {
            getId();
            Order order = Order.dao.findById(id, this.getCompanyId());
            Company company = Company.dao.qryCacheById(this.getCompanyId());
            Map<String, Object> config = company.getConfig();
            if (order.getInt("ordertype") == 2) {
                if ("true".equals(config.getOrDefault("p_sale_audit", "false")) && "3".equals(config.getOrDefault("p_sale_audit_type", "0"))) {
                    auditor_id = String.valueOf(config.getOrDefault("p_finance", this.getCurrentUserId()));
                }
            } else if (order.getInt("ordertype") == 3) {
                if ("true".equals(config.getOrDefault("p_saletui_audit", "false")) && "3".equals(config.getOrDefault("p_saletui_audit_type", "0"))) {
                    auditor_id = String.valueOf(config.getOrDefault("p_financetui", this.getCurrentUserId()));
                }
            }
            if (auditor_id.equals("")) {
                Order.dao.subAudit(id, this.getCompanyId(), this.getCurrentUserId(), dateTimeFormat.format(new Date()), status);
            } else {
                Order.dao.subAuditByUid(id, this.getCompanyId(), this.getCurrentUserId(), dateTimeFormat.format(new Date()), status, auditor_id);
            }
    ...
    public void save() {
        try {
            ...
            for (int i = 0; i < productlistlength; i++) {
                ...
                if(Integer.parseInt(String.valueOf(attr.get("amount")))<=0){
                    this.rendJson(false, null, "请核对数量,第"+i+"行数量存在负数");
                    return;
                }
            ...
            if(ordertype == 2){
                orderSend = (OrderSend)this.getModel2(OrderSend.class);
            }
            if (StringUtils.isEmpty(id)) {
                ...
                ldata.set("id", id);
                ldata.save();
                if(orderSend != null && orderSend.get("people") != null && !orderSend.get("people").toString().isEmpty()){
                    orderSend.set("order_id", id);
                    orderSend.save();
                }
            } else {
                po.update();
    ...
    //todo 该位置用于订单提交，会生成应收应付
    public void submit() {
        try {
            this.getId();
            String company_id = this.getCompanyId();
            Order order = Order.dao.findByIdOnlyOrder(id, company_id);
            if (order == null) {
    ...
    public void saveAudit() {
        try {
            ...
            if (po.getInt("audit_status") == 2 && ordertype != 4) {//不为报价单
                ...
                //生成对应的出入库单
                StorageBill.dao.createFromOrder(order, now, storageType);
                //更新销售目标
                String[] s = now.split("-");
                if (ordertype == 2 || ordertype == 3) {//销售或退货
                    BigDecimal zero = new BigDecimal(0);
            ...
            if (po.getInt("audit_status") == 2 && order.getInt("ordertype") == 4) {//报价单
                //转成订单
                Order.dao.tansfer(id, company_id,this.getCurrentUser());
            }
