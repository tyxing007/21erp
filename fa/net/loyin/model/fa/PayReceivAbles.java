...
public class PayReceivAbles extends Model<PayReceivAbles> {
	...
	public Page<PayReceivAbles> pageGrid(Integer pageNo,Integer pageSize,Map<String,Object> filter,Integer qryType){
		...
		String customer=(String)filter.get("customer");
		...
		return paginate(pageNo,pageSize,"select t.*,o.billsn,o.ordertype,o.id as orderid,cst.name customer_name,cst.type csttype,u.realname as creater_name,t.head_id,u1.realname as head_name ",sql.toString(), parame.toArray());
	}
	 ...
	 * @return
	 */
	public PayReceivAbles findByOrderId(String orderId,String company_id){
		StringBuffer sql=new StringBuffer("select *  from "+tableName+" t where t.order_id = ? and t.company_id = ? ");
		return dao.findFirst(sql.toString(),orderId,company_id);
	}
	...
	public String createFromOrder(Order order,String now){
		...
		Integer type=0;
		if(ordertype==0 || ordertype ==3){//采购订单、销售退货
			type=0;//付款
		}
		if(ordertype==1 || ordertype ==2){//销售订单 采购退货
			type=1;//收款
		}
...
//		}
		...
		//删除已有的应收应付
//		PayReceivAbles.dao.findById()
		PayReceivAbles po = this.findByOrderId(order_id,company_id);
		if(po == null){
			po=new PayReceivAbles();
...
//		}
	 ...
	 * @return
	 */
	public PayReceivAbles qryPayAblesByOrderId(String orderId) {
	 ...
	 * @return
	 */
	public List<Record> qryPayAblesCustomer(String companyId,String search) {
		String sql = "select distinct t.customer_id as id,c.name from "+tableName+ " t left join "+Customer.tableName+" c on c.id = t.customer_id where t.company_id=? ";
		List<Object> parame=new ArrayList<Object>();
		...
		return Db.find(sql, parame.toArray());
	}
