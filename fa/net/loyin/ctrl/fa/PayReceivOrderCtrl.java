...
public class PayReceivOrderCtrl extends AdminBaseController<PayReceivOrder> {
	...
	public void getCustomer(){
		String search = this.getPara("keyword");
		this.rendJson(true, null, "success", PayReceivOrder.dao.qryPayOrderCustomer(this.getCompanyId(),search));
	}
	...
	public void save(){
		try {
			...
			Integer status=this.getParaToInt("status");
			if(status == 0){
				this.rendJson(false,null, "您选择的为未结算，请重新选择！");
				return;
			}
			this.getId();
			...
			PayReceivAbles pay = PayReceivAbles.dao.findById(po.get("payables_id"));
			if(pay != null){
				StorageBill storageBill = StorageBill.dao.getStorageBillByOrderId(pay.get("order_id"));
				if(storageBill != null && storageBill.getInt("status")==4){
					storageBill.set("status",5);
					storageBill.update();
				}
			}

//			if(status==1 || status == 2){
				PayReceivOrder.dao.jiesuan(po.getBigDecimal("amt"),po.getStr("payables_id"),id,status);
//			}
